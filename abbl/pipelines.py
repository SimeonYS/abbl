...
class AabblPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('abbl.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `abbl`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM abbl WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO abbl (date, title, link, content)"
